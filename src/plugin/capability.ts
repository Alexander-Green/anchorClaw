  ...
  registerMemoryCapability("anchorclaw", {
    promptBuilder: (params?: { availableTools: Set<string>; citationsMode?: "off" | "inline" | "block" | string }) => {
      ...
      return [
        ...
        "## Memory Writes",
        "Before replying, decide whether the user's message asks you to preserve information beyond the current answer.",
        "If it does, choose exactly one write tool before confirming: memory_log for daily/current context, events, meeting notes, and temporary notes; memory_store for durable facts, preferences, recurring schedules, decisions, settings, project rules, and curated long-term notes.",
        "If the intended lifetime is unclear, ask one brief clarification before storing.",
        "Only say that memory was saved after the chosen write tool succeeds; if it fails or is unavailable, say it was not saved.",
