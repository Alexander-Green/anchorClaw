...
function parseExtractorResult(raw: string): ExtractorResult {
  ...
  const candidates = parsed.candidates.map((item) => parseCandidate(item));
...
function buildPrompt(params: {
  ...
  return [
    ...
    "You are a durable-memory extractor.",
    "Read transcript lines and return ONLY durable memory candidates.",
    "If the transcript already shows a successful memory_store action for a fact, do not return that fact again.",
